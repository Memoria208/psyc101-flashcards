# PSYC 101 Flashcards

An interactive flashcard study app for **PSYC 101: Introduction to Psychology**, Summer 2026.

🔗 **[Memoria208.github.io/psyc101-flashcards](https://Memoria208.github.io/psyc101-flashcards)**

---

## What it does

- **Flip cards** — tap or click any card to reveal the definition
- **Track progress** — mark cards as "Know it" or "Still learning" and watch a progress bar fill
- **Filter by topic** — chips at the top let you focus on one category (e.g. Research Methods, History, Ethics)
- **Chapter selector** — choose one chapter, several, or all of them before you start
- **Finals mode** — select all available chapters to study the whole semester at once
- **Dark mode** — automatically follows your system preference
- **No login, no account** — just open the page and study

---

## How to use it

1. **Open the app** → [Memoria208.github.io/psyc101-flashcards](https://Memoria208.github.io/psyc101-flashcards)
2. On the home screen, **click the chapters** you want to study (they highlight green when selected)
3. Hit **Start flashcards**
4. **Tap a card** to flip it and see the definition
5. Mark it **Know it ✓** or **Still learning ↺**
6. Use the **filter chips** to drill into a specific topic within your session
7. When you finish the deck, hit **Study again** to reshuffle, or **Choose chapters** to go back

---

## Chapters

| Chapter | Topic | Status |
|---------|-------|--------|
| Chapter 1 | Psychology's Roots, Critical Thinking & Self-Improvement | ✅ 44 cards |
| Chapter 2 | The Biology of Behavior and Consciousness | ✅ 65 cards |
| Chapter 3 | Developing Through the Life Span | ✅ 51 cards |
| Chapter 5 | Sensation & Perception | 🔜 Coming soon |
| Chapter 6 | Learning | 🔜 Coming soon |
| Chapter 7 | Memory | 🔜 Coming soon |
| Chapter 8 | Thinking, Language & Intelligence | 🔜 Coming soon |
| Chapter 9 | Motivation & Emotion | 🔜 Coming soon |
| Chapter 10 | Social Psychology | 🔜 Coming soon |
| Chapter 11 | Personality | 🔜 Coming soon |
| Chapter 12 | Psychological Disorders | 🔜 Coming soon |
| Chapter 13 | Therapy | 🔜 Coming soon |
| Chapter 14 | Stress, Health & Human Flourishing | 🔜 Coming soon |

New chapters are added throughout the semester as content is covered.

---

## A note on copyright

Card content is derived from assigned course readings used for personal study purposes. Do not republish or redistribute the card content commercially.

---

## Contributing

If you're a classmate and want to help add chapters or spot a mistake, feel free to open an issue or pull request.
